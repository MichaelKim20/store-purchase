...
describe("TransactionPool", () => {
    ...
    it("Remove Test", async () => {
        ...
        for (let idx = 0; idx < length; idx++) {
            ...
            assert.strictEqual(tx[0].purchase_id, txs[idx].purchase_id);
            assert.strictEqual(tx[0].timestamp, txs[idx].timestamp);
            assert.strictEqual(tx[0].amount.toString(), txs[idx].amount.toString());
            assert.strictEqual(tx[0].franchisee_id, txs[idx].franchisee_id);
            assert.strictEqual(tx[0].user_email, txs[idx].user_email);
