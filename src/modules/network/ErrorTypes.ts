...
export function handleNetworkError(error: any): Error {
    if (
        ...
        switch (error.response.data.code) {
            case 400:
                return new BadRequestError(error.response.data.code, error.response.statusText, statusMessage);
            case 404:
                return new NotFoundError(error.response.data.code, error.response.statusText, statusMessage);
            default:
                return new NetworkError(error.response.data.code, error.response.statusText, statusMessage);
        }
    } else {
